  ...
  return {
    ...
    init: function ( nodes ) {
      ...
      for ( name in nodes ) {
        ...
        var node = document.getElementById(this[name].id);
        if ( node ) {
          node.parentNode.replaceChild(this[name], node);
        }
      }
    }
  };
...
var regLoading = / ?loading/g,
  ...
  pageDOM = new Tree({
    ...
    content: {type: "div", id: "content", className: "fullPage", parentNode: "wrapper"},
    buttons: {type: "div", id: "buttons", parentNode: "content", before: "contentWrap"},
    contentWrap: {type: "div", id: "contentWrap", parentNode: "content", before: "modalOverlay", after: "buttons"},
    output: {type: "div", id: "output", parentNode: "contentWrap", after: "input"},
    modalOverlay: {type: "div", id: "modalOverlay", parentNode: "content", after: "*"},
    menuItems: {type:"select", parentNode: "modal", before: "modalButtons", after: "modalTitleInput"},
    menuGroup: {type:"optgroup", parentNode: "menuItems"}
  }),
  md = markdownit({
...
function Item ( page ) {
  return {
    ...
    init: function ( page ) {
      ...
      this.contentType = page.contentType || "Content";
      this.listItemType = page.listItemType || "";
      this.timestamp = page.timestamp || null;
      this.inTransition = page.inTransition || {};
      this.animating = page.animating || false;
    }
  };
}
...
function newModal ( options, cb ) {
  ...
  $modalOverlay.velocity({ opacity: 1 }, { duration: 75, display: "block" });
    ...
    complete: function () {
      ...
      if ( !CodeMirror ) {
        fastdom.write(function() {
          contentWrap.appendChild(output);
          try { loadingSomething(false, output) } catch (e) {
            console.log("output loading detach fail: ", e);
            try { loadingSomething(false, contentWrap); } catch ( e ) {
              console.log("contentWrap loading detach fail: ", e);
            } }
        });
      }
      else {
