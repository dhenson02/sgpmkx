(function e(t, n, r) {
    ...
    1: [ function(require, module, exports) {
        (function(window, document, reqwest, Route, undefined) {
            ...
            function render() {
                return h("#wrapper", [ h("#sideNav", [ app.navDOM ]), h("#content.fullPage", [ h("#contentWrap", [ h("#output") ]) ]) ]);
            }
            function renderEditor() {
                return h("#wrapper", [ h("#sideNav", [ app.navDOM ]), h("#content.fullPage", [ h("#buttons", [ h("#toggleButton.btn", {
                    ...
                    type: "text"
                }) ]), h("textarea#textarea", [ String(app.currentContent.text || "") ]) ]), h("#output") ]) ]) ]);
            }
            ...
            function getList() {
                reqwest({
                    ...
                    success: function(data) {
                        function handleNav() {
                            var oldActive = app.rootNode.querySelectorAll("a.active"), i = 0, count = oldActive.length;
                            for (;i < count; ++i) {
                                oldActive[i].className = oldActive[i].className.replace(/ ?active/g, "");
                            }
                            this.className = "active";
                        }
                        var results = data.d.results, pages = {}, fhm = [], fhmLinks = [], comm = [], commLinks = [], i = 0, count = results.length, page;
                        for (;i < count; ++i) {
                            page = results[i];
                            ...
                            if (/^\/fhm\//i.test(page.Category)) {
                                ...
                                fhmLinks.push(h("li", [ h("a", {
                                    href: "#" + page.Category,
                                    onclick: handleNav
                                }, [ String(page.Title), h("span") ]), h("hr") ]));
                            }
                            if (/^\/comm\//i.test(page.Category)) {
                                comm.push(h("option", {
                                    value: page.Category
                                }, [ String(page.Title) ]));
                                commLinks.push(h("li", [ h("a", {
                                    href: "#" + page.Category,
                                    onclick: handleNav
                                }, [ String(page.Title), h("span") ]), h("hr") ]));
                            }
